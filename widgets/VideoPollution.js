    ...
    var video = declare("widgets.VideoPollution", _Widget, {
        ...
        _cutData1: function (data) {
            ...
            (function (that, popup, tableContentId) {
                ...
                ne.data = data; //数据；注：数据格式为[{},{},{}];外层为数组，每个数组内的元素为对象的格式；
                ne.pageContentLength = 10; //每页展示数据的长度或个数；
                ne.orderKey = ["itemId", "name", "Contact", "Phone"]; //表格展示的属性名的顺序
                ne.orderKeyName = ["序号", "名称", "联系人", "联系电话"]; //表格头部的属性名一次的名称与orderKey对应起来；
                ne.addDom({
                    that: that,
                    popup: popup
                }); //调用主方法；
            ...
            if (tableId.innerHTML != undefined && tableId.innerHTML != null) {
                ...
                for (var i = 1; i < w_cons.length; i++) {
                    ...
                    w_cons[i].onclick = function () {
                        ...
                        var ckItem = controls[this.index - 1];
                        var graphic = (function () {
                            var graphics = that._videoPollutionlayer.graphics;
                            for (var i = 0, length = graphics.length; i < length; i++) {
                                var item = graphics[i];
                                if (item.attributes.attributesData.id === ckItem.id) {
                                    return item;
                                }
                            }
                        })();
        ...
        _createSearch: function () {
            ...
            var lsTypes = [{ name: '餐饮', id: 1 }, { name: '扬尘', id: 2 }, { name: 'VOC企业', id: 3 }, { name: '小散乱污企业', id: 4 }];
            ...
            var lsStates = [{ name: '请选择', id: -1 }, { name: '运行中', id: 0 }, { name: '异常', id: 1 }];//1异常，0运行中
            ...
            setTimeout(function () {
                ...
                dojo.connect(dojo.byId('cm-type-camera'), 'onclick', function (e) {
                    var cmStateElement = dojo.byId('cm-status-camera');
                    cmStateElement.style.display = '';
                    var cmCheckedElement = e.currentTarget;
                    var cmType = cmCheckedElement.selectedIndex + 1;
                    (parseInt(cmType) === 2 || parseInt(cmType) === 4) && (cmStateElement.style.display = 'none', cmStateElement.selectedIndex = 0);
                    //console.log(cmStateElement);
                });
            }, 100);
        ...
        _searchCamera: function (t) {
            ...
            xhr.post(t._projectName + "/widgets/handler/VideoPollution.ashx", {
                ...
                var data = eval("(" + data + ")");
                //if (data.length > 0) {
                //    if (data[0].Type == "2" || data[0].Type == "4") {//如果是扬尘和小散乱污企业
                //        t._cutData1(data);
                //    } else {
                //        t._cutData(data);
                //    }
                ...
                //    t._cutData(data);
                //}
                if (parseInt(cmTypeAttr) === 2 || parseInt(cmTypeAttr) === 4) {
                    t._cutData1(data);
                } else {
                    t._cutData(data);
                }
        ...
        _setLayerVisible: function () {
            ...
            var camera = document.getElementById('cm-type-camera');
            var stateElement = document.getElementById('cm-status-camera');
            camera && (camera.selectedIndex = 0);
            stateElement && (stateElement.selectedIndex = 0, stateElement.style.display = '');
        }
    });
    return video;
});