...
define([
    ...
    "dojo/on",
    "dijit/popup",
    "dojo/text!./templates/ExposedLandPollution.html",
    "dijit/TooltipDialog",
    "dijit/form/CheckBox",
    ...
    var exposedLand = declare("widgets.ExposedLandPollution", _Widget, {
        ...
        _setEvent: function () {
            ...
            on(this.checkBox, "change", lang.hitch(this, function (evt) {
                ...
                if (this.checkBox.checked) {
                    ...
                    this._clearTable()
                    this._exposedLandPollutionlayer.clear();
                    popup.close()
                }
            }));
        },
        ...
        _addGraphics: function () {
            this._exposedLandPollutionlayer = this.map.getLayer("exposedLandPollution");
            xhr.post(this._projectName + "/widgets/handler/ExposedLandPollution.ashx", {
                handleAs: "text",
                timeout: 10000,
                data: {
                    methodName: "GetAllExposedLandAreas"
                }
            }).then(lang.hitch(this, function (data) {
                var dataJson = eval("(" + data + ")");
                this._cutData(dataJson)
                this._addpointAll(dataJson)
        ...
        _cutData: function (data) {
            ...
            })(this, popup, this.tableContentId + 7)
        },
        //首次数据加载为图中坐标点添加事件与数据；
        _setLayer: function (data) {
            this._exposedLandPollutionlayer = this.map.getLayer("exposedLandPollution");
            this._exposedLandPollutionlayer.on("click", lang.hitch(this, function (evt) {
                this._mapPoint = evt.mapPoint;
                var nowdata = data[evt.graphic.attributes.exposedLandCode]
        ...
        _addpointAll: function (data) {
            this._exposedLandPollutionlayer = this.map.getLayer("exposedLandPollution");
            this._exposedLandPollutionlayer.clear()
            for (var exposedLand in data) {
                var point = new Point(data[exposedLand].经度, data[exposedLand].纬度, this.map.spatialReference);
                var symbol = new PictureMarkerSymbol(this._projectName + "/widgets/assets/images/exposedLand.png", 20, 20);
                var attr = { "exposedLandCode": exposedLand };
                var graphic = new Graphic(point, symbol, attr, null);
                this._exposedLandPollutionlayer.add(graphic);
                var textSymbol = new TextSymbol({
                    text: data[exposedLand].名称,
                    font: "12px",
                    color: new Color("red"),
        ...
        _pointadd: function (controls) {
            this._tableshow()
            var that = this;
            var tableId = dojo.byId(this.tableContentId + 7)
            if (tableId.innerHTML != undefined && tableId.innerHTML != null) {
                ...
                for (var item = 1; item < w_cons.length; item++) {
                    ...
                    w_cons[item].onclick = function () {
                        ...
                        that._exposedLandPollutionlayer = that.map.getLayer("exposedLandPollution");
                        topic.publish("widgets_ExposedLandPollution_0" + "click", that._mapPoint, that._toolTipDialog);
                        var attrData = [];
                        var attrIndex = 0;
        ...
        //功能面板与表格面板的切换；
        TabHost: function () {
            var $tableTitleId = dojo.byId(this.tableTitleId + 7);//表格头部对应的标题
            this.tableTitleNode.style.display = "block"//表格头部
            $tableTitleId.style.display = "inline-block"
            $tableTitleId.innerHTML = "料土堆列表";
        ...
        _tableshow: function () {
            ...
            $tableTitle[7].style.background = "#f1ab00";
            $tableTitle[7].style.borderRight = "1px solid beige";
            $tableTitle[7].style.borderBottom = "1px solid beige";
            $tableContent[7].style.display = "block";
            for (var i = 0; i < $tableTitle.length; i++) {
        ...
        _clearTable: function () {
            var $tableTitleId = dojo.byId(this.tableTitleId + 7);
            var $tableContentId = dojo.byId(this.tableContentId + 7);
        ...
        _setLayerVisible: function () {
            var layer = this.map.getLayer("exposedLandPollution");
            layer.setVisibility(this.checkBox.checked);
            this.textLayer = this.map.getLayer("exposedLandTextSymbol");
