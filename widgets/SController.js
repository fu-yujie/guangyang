...
define([
    ...
    "dojo/topic",
    "dojo/on",
    "dojo/text!./templates/SController.html",
    "dijit/form/Button",
    "dijit/form/CheckBox",
    "dijit/popup",
    "dijit/TooltipDialog",
    ...
    var t = declare("widgets.SController", _Widget, {
        map:null,
        ...
        _getData: function () {
            ...
            /// 获取所有微监控点的属性信息
            /// </summary>
            xhr.post(this._projectName + "/widgets/handler/SController.ashx", {
                handleAs: "text",
                data: {
                    methodName: "GetAllSController"
                }
            }).then(lang.hitch(this, function (sController) {
                this._cutData(eval("(" + sController + ")"))
                this._addpoint(eval("(" + sController + ")"))
                this._setLayer(eval("(" + sController + ")"))
        ...
        _setLayer: function (data) {
            this._sControllerLayer = this.map.getLayer("sController");
            this._sControllerLayer.on("click", lang.hitch(this, function (evt) {
            ...
            if (length > 0) {
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    switch (data[i].StatusName) {
                        case "优":
                            {
                                this._url = this._projectName + "/widgets/assets/images/SController_green.png";
                            }
                            break;
                        case "良":
                            {
                                this._url = this._projectName + "/widgets/assets/images/SController_yellow.png";
                            }
                            break;
                        case "轻度":
                            {
                                this._url = this._projectName + "/widgets/assets/images/SController_orange.png";
                            }
                            break;
                        case "中度":
                            {
                                this._url = this._projectName + "/widgets/assets/images/SController_red.png";
                            }
                            break;
                        case "重度":
                            {
                                this._url = this._projectName + "/widgets/assets/images/SController_purple.png";
                            }
                            break;
                        case "严重":
                            {
                                this._url = this._projectName + "/widgets/assets/images/SController_maroon.png";
                                console.log("严重污染")
                            }
                            break;
                        default:
                    ...
                    this._sControllerLayer.add(graphic);
                }
            }
        },
        //添加表格与图中坐标点数据展示联动
        ...
        _setEvent:function(){
            ...
            on(this.checkBox,"change",lang.hitch(this,function(evt){
                ...
                if (this.checkBox.checked) {
                    this._getData()
                } else if (this._sControllerLayer != undefined) {
                    // 清除图中的元素                      
                    this._clearTable()
                    popup.close()
                }
        ...
        _setLayerVisible:function() {
            var layer = this.map.getLayer("sController");
            ...
            //        if(graphic.attributes.type == "sController"){
