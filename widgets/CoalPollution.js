...
define([
    ...
    "dojo/on",
    "dijit/popup",
    "dojo/text!./templates/CoalPollution.html",
    "dijit/TooltipDialog",
    "dijit/form/CheckBox",
    ...
    var coal = declare("widgets.CoalPollution", _Widget, {
        ...
        _setEvent: function () {
            ...
            on(this.checkBox, "change", lang.hitch(this, function (evt) {
                ...
                if (this.checkBox.checked) {
                    ...
                    this._coalPollutionlayer.clear()
                    this._clearTable()
                    popup.close();
        ...
        _addGraphics: function () {
            this._coalPollutionlayer = this.map.getLayer("coalPollution");
            this._coalPollutionlayer.clear();
            xhr.post(this._projectName + "/widgets/handler/CoalPollution.ashx", {
                handleAs: "text",
                timeout: 10000,
                data: {
                    methodName: "GetAllCoalAreas"
                }
        ...
        _setLayer: function (data) {
            this._coalPollutionlayer = this.map.getLayer("coalPollution");
            this._coalPollutionlayer.on("click", lang.hitch(this, function (evt) {
                this._mapPoint = evt.mapPoint;
                var nowdata = data[evt.graphic.attributes.coalCode]
        ...
        _addpointAll: function (data) {
            this._coalPollutionlayer = this.map.getLayer("coalPollution");
            this._coalPollutionlayer.clear()
            this.textLayer = this.map.getLayer("coalTextSymbol");
            this.textLayer.clear()
            for (var coal in data) {
                var point = new Point(data[coal].经度, data[coal].纬度, this.map.spatialReference);
                var symbol = new PictureMarkerSymbol(this._projectName + "/widgets/assets/images/coal.png", 20, 20);
                var attr = { "coalCode": coal };
                var graphic = new Graphic(point, symbol, attr, null);
                this._coalPollutionlayer.add(graphic);
            ...
            if (tableId.innerHTML != undefined && tableId.innerHTML != null) {
                ...
                for (var item = 1; item < w_cons.length; item++) {
                    ...
                    w_cons[item].onclick = function () {
                        ...
                        that._coalPollutionlayer = that.map.getLayer("coalPollution");
                        topic.publish("widgets_CoalPollution_0" + "click", that._mapPoint, that._toolTipDialog);
                        var attrData = [];
                        var attrIndex = 0;
                        var resultJson = controls[this.index - 1]
                        for (var item in resultJson) {
                            attrIndex++;
                            if (item != "经度" && item != "纬度" && item != "所属网格") {
                                attrData.push({
                                    ID: attrIndex,
                                    name: item,
        ...
        _tableshow: function () {
            ...
            $tableTitle[1].style.borderRight = "1px solid beige";
            $tableTitle[1].style.borderBottom = "1px solid beige";
        ...
        _setLayerVisible: function () {
            var layer = this.map.getLayer("coalPollution");
