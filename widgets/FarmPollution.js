...
define([
    ...
    "dojo/on",
    "dijit/popup",
    "dojo/text!./templates/FarmPollution.html",
    "dijit/TooltipDialog",
    "dijit/form/CheckBox",
    ...
    var farm = declare("widgets.FarmPollution", _Widget, {
        ...
        _setEvent: function () {
            ...
            on(this.checkBox, "change", lang.hitch(this, function (evt) {
                ...
                if (this.checkBox.checked) {
                    ...
                    this._farmPollutionlayer.clear();
                    this._clearTable()
                    popup.close();
                }
            }));
        ...
        _setGrid: function () {
            this._attrGrid = new DataGrid({
                style: "padding:0;font-size: 12px;width:220px;height:200px;font-family: 'Microsoft YaHei';float:left;",
        ...
        _addGraphics: function () {
            this._farmPollutionlayer = this.map.getLayer("farmPollution");
            this._farmPollutionlayer.clear();
            xhr.post(this._projectName + "/widgets/handler/FarmPollution.ashx", {
                handleAs: "text",
                timeout: 10000,
                data: {
                    methodName: "GetAllFarm"
                }
        ...
        _cutData: function (data) {
            ...
            })(this, popup, this.tableContentId + 5)
        },
        //首次数据加载为图中坐标点添加事件与数据；
        _setLayer: function (data) {
            this._farmPollutionlayer = this.map.getLayer("farmPollution");
            this._farmPollutionlayer.on("click", lang.hitch(this, function (evt) {
                this._mapPoint = evt.mapPoint;
                var nowdata = data[evt.graphic.attributes.farmCode]
        ...
        _addpointAll: function (data) {
            this._farmPollutionlayer = this.map.getLayer("farmPollution");
            this._farmPollutionlayer.clear()
            this.textLayer = this.map.getLayer("farmTextSymbol");
            this.textLayer.clear()
            for (var farm in data) {
                var point = new Point(data[farm].经度, data[farm].纬度, this.map.spatialReference);
                var symbol = new PictureMarkerSymbol(this._projectName + "/widgets/assets/images/farmPollution.png", 20, 20);
                var attr = { "farmCode": farm };
                var graphic = new Graphic(point, symbol, attr, null);
                this._farmPollutionlayer.add(graphic);
        ...
        _pointadd: function (controls) {
            this._tableshow()
            var that = this;
            var tableId = dojo.byId(this.tableContentId + 5)
            if (tableId.innerHTML != undefined && tableId.innerHTML != null) {
                ...
                for (var item = 1; item < w_cons.length; item++) {
                    ...
                    w_cons[item].onclick = function () {
                        ...
                        that._farmPollutionlayer = that.map.getLayer("farmPollution");
                        topic.publish("widgets_FarmPollution_0" + "click", that._mapPoint, that._toolTipDialog);
                        var attrData = [];
                        var attrIndex = 0;
        ...
        //功能面板与表格面板的切换；
        TabHost: function () {
            var $tableTitleId = dojo.byId(this.tableTitleId + 5);//表格头部对应的标题
            this.tableTitleNode.style.display = "block"//表格头部
            $tableTitleId.style.display = "inline-block"
            $tableTitleId.innerHTML = "养殖场列表";
        ...
        _tableshow: function () {
            ...
            $tableTitle[5].style.background = "#f1ab00";
            $tableTitle[5].style.borderRight = "1px solid beige";
            $tableTitle[5].style.borderBottom = "1px solid beige";
            $tableContent[5].style.display = "block";
            for (var i = 0; i < $tableTitle.length; i++) {
        ...
        _clearTable: function () {
            var $tableTitleId = dojo.byId(this.tableTitleId + 5);
            var $tableContentId = dojo.byId(this.tableContentId + 5);
        ...
        secondaryFilter: function (dataJsons) {
            ...
            var lab = domConstruct.create("p", {
                ...
                id: this.tableContentId + 5 + "_p"
            });
            dojo.byId(this.tableContentId + 5).insertBefore(lab, dojo.byId(this.tableContentId + 5).firstElementChild)
            lab.innerHTML += "<span style='float:left;width:120px;height:25px;text-align:center;'><input type='checkbox' style='width:15px;height:15px;vertical-align:bottom;display:inline-block'/><em style='font-size:12px;'>" + "远离水源" + "</em></span>"
            lab.innerHTML += "<span style='float:left;width:120px;height:25px;text-align:center;'><input type='checkbox' style='width:15px;height:15px;vertical-align:bottom;display:inline-block'/><em style='font-size:12px;'>" + "靠近水源" + "</em></span>"
        ...
        _clik: function (dt) {
            var that = this;
            var dat = dt
            var $p = dojo.byId(this.tableContentId + 5 + "_p");
        ...
        _setLayerVisible: function () {
            var layer = this.map.getLayer("farmPollution");
