...
define([
    ...
    "dojo/on",
    "dijit/popup",
    "dojo/text!./templates/CarPollution.html",
    "dijit/TooltipDialog",
    "dijit/form/CheckBox",
    ...
    var car = declare("widgets.CarPollution", _Widget, {
        ...
        _addGraphics: function () {
            this._carPollutionlayer = this.map.getLayer("carPollution");
            this._carPollutionlayer.clear();
            xhr.post(this._projectName + "/widgets/handler/CarPollution.ashx", {
                handleAs: "text",
                timeout: 10000,
                data: {
                    methodName: "GetAllCarAreas"
                }
        ...
        _cutData: function (data) {
            ...
            (function (that, popup, tableContentId) {
                ...
                ne.orderKey = ["名称", "编码", "联系人"];//表格展示的属性名的顺序
                ne.orderKeyName = ["污染源名称", "编码", "联系人"]//表格头部的属性名一次的名称与orderKey对应起来；
                ne.addDom({
                    that: that,
                    popup: popup
                })//调用主方法；
                ne.addOrRemove("set", "border", 0)//第一个属性为remove和set，第二属性为标签的属性名，第三个属性为属性值；
            })(this, popup, this.tableContentId + 9)
        },
        //首次数据加载为图中坐标点添加事件与数据；
        _setLayer: function (data) {
            this._carPollutionlayer = this.map.getLayer("carPollution");
            this._carPollutionlayer.on("click", lang.hitch(this, function (evt) {
                this._mapPoint = evt.mapPoint;
                var nowdata = data[evt.graphic.attributes.carCode]
                //this.map.setExtent(this._mapPoint);
                topic.publish(this.domNode.id + "click", this._mapPoint, this._toolTipDialog);
                var attrData = [];
                var attrIndex = 0;
                for (var item in nowdata) {
                    attrIndex++;
                    if (item != "纬度" && item != "经度" && item != "所属网格" && item != "地址") {
                        attrData.push({
                            ID: attrIndex,
                            name: item,
                            value: nowdata[item]
        ...
        _addpointAll: function (data) {
            this._carPollutionlayer = this.map.getLayer("carPollution");
            this._carPollutionlayer.clear()
            this.textLayer = this.map.getLayer("carTextSymbol");
            this.textLayer.clear()
            for (var car in data) {
                var point = new Point(data[car].经度, data[car].纬度, this.map.spatialReference);
                var symbol = new PictureMarkerSymbol(this._projectName + "/widgets/assets/images/car.png", 20, 20);
                var attr = { "carCode": car };
                var graphic = new Graphic(point, symbol, attr, null);
                this._carPollutionlayer.add(graphic);
        ...
        _pointadd: function (controls) {
            this._tableshow()
            var that = this;
            var tableId = dojo.byId(this.tableContentId + 9)
            if (tableId.innerHTML != undefined && tableId.innerHTML != null) {
                ...
                for (var item = 1; item < w_cons.length; item++) {
                    ...
                    w_cons[item].onclick = function () {
                        ...
                        that._carPollutionlayer = that.map.getLayer("carPollution");
                        topic.publish("widgets_CarPollution_0" + "click", that._mapPoint, that._toolTipDialog);
                        var attrData = [];
                        var attrIndex = 0;
                        var resultJson = controls[this.index - 1]
                        for (var item in resultJson) {
                            attrIndex++;
                            if (item != "经度" && item != "纬度" && item != "所属网格"&&item!="地址") {
                                attrData.push({
                                    ID: attrIndex,
                                    name: item,
        ...
        //功能面板与表格面板的切换；
        TabHost: function () {
            var $tableTitleId = dojo.byId(this.tableTitleId + 9);//表格头部对应的标题
            this.tableTitleNode.style.display = "block"//表格头部
            $tableTitleId.style.display = "inline-block"
            $tableTitleId.innerHTML = "大车列表";
        ...
        _tableshow: function () {
            ...
            $tableTitle[9].style.background = "#f1ab00";
            $tableTitle[9].style.borderRight = "1px solid beige";
            $tableTitle[9].style.borderBottom = "1px solid beige";
            $tableContent[9].style.display = "block";
            for (var i = 0; i < $tableTitle.length; i++) {
        ...
        _clearTable: function () {
            var $tableTitleId = dojo.byId(this.tableTitleId + 9);
            var $tableContentId = dojo.byId(this.tableContentId + 9);
        ...
        _setLayerVisible: function () {
            var layer = this.map.getLayer("carPollution");
